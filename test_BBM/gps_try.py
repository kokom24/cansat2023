...
IM920Serial = serial.Serial('/dev/ttyAMA0', 19200)  # シリアルポートとボーレートを設定（ポートは適宜変更してください）
...
def send_data(data):
    ...
    print("データを送信しました:", data)

def send_data():
    IM920Serial.write((f"TXDU 0001,1000" + '\r\n').encode())
    print("データを送信しました:")

def send_gps():
    IM920Serial.write((f"TXDU 0001,{lat},{lon}"+ '\r\n').encode())
    print("データを送信しました:",lat,lon)

def send_lon(lon):
    IM920Serial.write(("TXDU 0001," +lon+ '\r\n').encode())
    print("データを送信しました:",lon)

def send_lat(lat):
    IM920Serial.write(("TXDU 0001," +lat+ '\r\n').encode())
    print("データを送信しました:",lat)
        ...
        while True:
            ...
            if utc == -1.0:
                if lat == -1.0:
                    ...
                    #0000はreading gps error
                    send_data("TXDU 0001,0000")
                    # pass
                else:
                    # pass
                   print("Status V")
                   #0001はstatus v
                   send_data("TXDU 0001,0001")
                   #send_data_ver2()
            else:
                # pass
                ...
                # send_data_ver2(lat)
                # send_data_ver2(lon)
                # send_data_ver2(sHeight)
                # send_data_ver2(gHeight)
            time.sleep(10)
    except KeyboardInterrupt:
        close_gps()
        print("\r\nKeyboard Intruppted, Serial Closed")
